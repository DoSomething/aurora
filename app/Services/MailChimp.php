...
use Drewm\MailChimp as MailChimpAPI;
...
class MailChimp
{
    ...
    $this->dinosaurID = config('services.mailchimp.dinosaur_id');
        ...
        $this->client = new MailChimpAPI(config('services.mailchimp.apikey'));
    }

    public function unsubscribe($email, $list_id)
    {
        $response = $this->client->call('lists/unsubscribe', [
      'id' => $list_id,
      'email' => ['email' => $email],
    ]);
    ...
    public function listFinder($email)
    {
        ...
        foreach ($list_ids as $list_id) {
            $response = $this->client->call('lists/member-info', ['id' => $list_id, 'emails' => [['email' => $email]]]);
